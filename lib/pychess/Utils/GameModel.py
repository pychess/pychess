...
class GameModel(GObject.GObject):
    ...
    def _get_curplayer(self):
        try:
            ...
            log.error("{} {}".format(self.players, self.getBoardAtPly(self.ply).color))
            raise
    ...
    def _get_waitingplayer(self):
        try:
            ...
            log.error("{} {}".format(self.players, 1 - self.getBoardAtPly(self.ply).color))
            raise
    ...
    def offerReceived(self, player, offer):
        log.debug("GameModel.offerReceived: offerer={} {}".format(repr(player), offer))
    ...
    def withdrawReceived(self, player, offer):
        log.debug(
            f"GameModel.withdrawReceived: withdrawer={repr(player)} {offer}"
        )
        if player == self.players[WHITE]:
            opPlayer = self.players[BLACK]
        else:
    ...
    def declineReceived(self, player, offer):
        log.debug(
            f"GameModel.declineReceived: decliner={repr(player)} {offer}"
        )
        if player == self.players[WHITE]:
            opPlayer = self.players[BLACK]
        else:
    ...
    def acceptReceived(self, player, offer):
        log.debug(
            f"GameModel.acceptReceived: accepter={repr(player)} {offer}"
        )
        if player == self.players[WHITE]:
            opPlayer = self.players[BLACK]
        else:
