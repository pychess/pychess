...
class Sidepanel(ParrentListSection):
    def load(self, widgets, connection, lounge):
        ...
        self.store = Gtk.ListStore(
            FICSGame, GdkPixbuf.Pixbuf, str, int, str, int, str, str
        )
        ...
        def searchCallback(model, column, key, sel_iter, user_data):
            if model.get_value(sel_iter, 2).lower().startswith(key) or model.get_value(
                sel_iter, 4
    ...
    def game_filter_func(self, model, iter, data):
        game = model[iter][0]
        is_standard = game.game_type.rating_type in (
            TYPE_STANDARD,
            TYPE_FIFTEEN_MINUTE,
            TYPE_FORTYFIVE_MINUTE,
        )
        is_blitz = game.game_type.rating_type in (
            TYPE_BLITZ,
            TYPE_THREE_MINUTE,
            TYPE_FIVE_MINUTE,
        )
        is_lightning = game.game_type.rating_type in (
            TYPE_LIGHTNING,
            TYPE_BULLET,
            TYPE_ONE_MINUTE,
        )
        ...
        return (
            (self.filter_toggles["standard_toggle"] and is_standard)
            or (self.filter_toggles["blitz_toggle"] and is_blitz)
            or (self.filter_toggles["lightning_toggle"] and is_lightning)
            or (self.filter_toggles["variant_toggle"] and is_variant)
        )
    ...
    def on_query_tooltip(self, widget, x, y, keyboard_tip, tooltip):
        ...
        if result is not None:
            ...
            for player, column_number in (
                (self.model[path][0].wplayer, 1),
    ...
    def onGameAdd(self, games, new_games):
        ...
        GLib.idle_add(
            do_onGameAdd, games, new_games, game_store, priority=GLib.PRIORITY_LOW
        )
