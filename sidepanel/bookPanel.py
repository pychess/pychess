...
class Sidepanel:
    ...
    def shown_changed (self, board, shown):
        ...
        if endings:
            self.tabulateEndings(b, endings)
        elif openings:
            self.tabulateOpenings(b, openings)
        else:
    ...
    def tabulateOpenings (self, board, openings):
        totalWeight = 0
        ...
        for move, weight, games, score in openings:
            ...
            history = board.color == WHITE and (w, l, games) or (l, w, games)
    ...
    def selection_changed (self, widget, *args):
        ...
        move = parseAny(b, self.store.get(iter, 0)[0])
